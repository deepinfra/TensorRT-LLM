...
std::shared_ptr<DecodingInputs> DynamicDecodeLayerTest<T>::createInputTensors(SizeType32 step)
{
    ...
    if (mDecodingMode.isTopKorTopP())
    {
        forwardParams = std::make_shared<SamplingInputs>(mEndIdsDevice, mMinPsDevice, step, ite, mBatchSize);
    }
    else if (mDecodingMode.isMedusa())
    {
        forwardParams = std::make_shared<MedusaDecodingInputs>(mEndIdsDevice, mMinPsDevice, mBatchSize);
    }
