    ...
    std::string filename, void const* src, std::size_t bytes, std::uint64_t spillId)
{
...

    {
        ...
        auto const blkNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
...
void KVCacheTransferManager::reportDiskProfLocked()
{
    ...
    TLLM_LOG_INFO("[disk-prof] %llds spills=%zu qpeak=%zu | enqBlk n=%zu tot=%.0fms max=%.0fms | "
    ...
    std::vector<KVCacheBlockPool> const& pools, std::string const& directory, std::uint64_t spillId)
{
    ...
    for (size_t poolIdx = 0; poolIdx < pools.size(); ++poolIdx)
    {
        ...
        enqueueDiskWriteNoCopy(filename, ptr->data(), bytes, spillId);
    }
}
