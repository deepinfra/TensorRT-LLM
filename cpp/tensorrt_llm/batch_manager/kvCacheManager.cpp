...
BlockManager::BlockManager(std::vector<SizeType32> const& numKvHeadsPerLayer, SizeType32 sizePerHead,
    SizeType32 tokensPerBlock, BlocksPerWindow const& blocksPerWindow,
    SizeType32 maxNumSequences, CudaStreamPtr stream,
    ...
    , mLinearAttentionMetadata{std::move(linearAttentionMetadata)}
{
    ...
    if (numPlaceholderBlocks > 0)
    {
        ...
        policy->initialize(mAllBlocksById, {blocksInPrimaryPool, blocksInSecondaryPool, blocksInDiskPool}, secondaryOffloadMinPriority);
    ...
    else
    {
        ...
        mEvictionPolicy->initialize(
            mAllBlocksById, {blocksInPrimaryPool, blocksInSecondaryPool, blocksInDiskPool}, secondaryOffloadMinPriority);
    }
    if (mEventManager)
    {
...
BlockPtr WindowBlockManager::reclaimSecondaryBlock()
{
    ...
    if (mNumDiskBlocks == 0 || victim->getUniqueTokens().empty()
        || mEvictionPolicy->getNumFreeBlocks(kDiskLevel) == 0)
    {
    ...
    { auto const* e = std::getenv("TLLM_KV_DISK_MIN_REUSE"); return e ? static_cast<SizeType32>(std::atoi(e)) : 0; }();
    ...
    // drop a BEST-EFFORT (unpaid) spill rather than stalling the scheduler behind the write queue. Paid/
    ...
    { auto const* e = std::getenv("TLLM_KV_DISK_DROP_ON_PRESSURE"); return e && std::atoi(e) != 0; }();
    if (kDropOnPressure && !victim->isRetainedNow() && mTransferManager->diskWriteQueueFull())
    {
        ...
        {
            TLLM_LOG_INFO("[disk-tier] write-pressure dropped=%zu (windowSize=%d)", mDiskWritePressureDropped,
                mWindowSize);
        }
    ...
    executor::KvCacheTransferMode mode, std::string const& directory)
{
    if (offloadBlock->isOnDisk())
    {
        ...
        auto const trackId = mTransferManager->asyncDiskReadEnabled()
...
KVCacheManager::KVCacheManager(SizeType32 numLayers, SizeType32 numKvHeads, SizeType32 sizePerHead,
    ...
    : KVCacheManager(std::vector<SizeType32>(numLayers, numKvHeads), sizePerHead, tokensPerBlock, blocksPerWindow,
        ...
        enableIndexerKCache, indexerKCacheQuantBlockSize, indexerKCacheIndexHeadDim, indexerKCacheUseFp4,
        linearAttentionMetadata, poolConfigurations, blocksInDiskPool, diskCachePath, diskRetainedOnly, diskProtectUnexpired)
{
}
...
KVCacheManager::KVCacheManager(std::vector<SizeType32> const& numKvHeadsPerLayer, SizeType32 sizePerHead,
    ...
    : KVCacheManager(numKvHeadsPerLayer, sizePerHead, tokensPerBlock, blocksPerWindow, maxNumSequences, maxBeamWidth,
        ...
        indexerKCacheUseFp4, linearAttentionMetadata, poolConfigurations, blocksInDiskPool, diskCachePath, diskRetainedOnly, diskProtectUnexpired)
{
}
