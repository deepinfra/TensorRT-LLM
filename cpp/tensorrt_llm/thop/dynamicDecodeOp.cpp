    ...
    th::optional<th::Tensor> beamHypsIsDoneOpt, bool const useBeamHyps)
{
    ...
    if (isBeamSearch)
    {
        forwardParams = std::make_shared<tl::DecodingInputs>(convert_tensor<int>(endId), convert_tensor<float>(minP), step, static_cast<int>(ite),
            localBatchSize, maxAttentionWindow, sinkTokenLength);
    }
    else
    {
        forwardParams = std::make_shared<tl::SamplingInputs>(
            convert_tensor<int>(endId), convert_tensor<float>(minP), step, static_cast<int>(ite), localBatchSize);
    }
