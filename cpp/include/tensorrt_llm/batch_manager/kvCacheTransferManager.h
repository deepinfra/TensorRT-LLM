...
public:
    ...
    void loadFromFile(BlockPtr const& dstPrimaryBlock, SizeType32 diskSlot,
        std::vector<KVCacheBlockPool> const& pools, std::string const& directory, std::int32_t trackBlockId = -1);
    ...
    //! \brief Disk tier (no-copy async): spill a host block's bytes by handing the writer the source
    ...
    void spillToFileNoCopy(BlockPtr const& srcHostBlock, SizeType32 diskSlot,
    ...
    //! the last call (no-copy path). The block manager reaps the corresponding pinned blocks.
    ...
    //! \brief True if the async write queue is at capacity. Used to shed best-effort (unpaid) spills under
    //! writer saturation; paid spills bypass the cap and are never shed.
...
private:
    ...
    struct DiskWriteJob
    {
        ...
        void const* src{nullptr};         // no-copy path: writer reads this pinned host pointer directly
        std::uint64_t spillId{0};         // >0 => track per-spill completion for the reserved-pool reap
        bool paid{false};                 // retained/paid spill: bypasses the queue cap, never dropped
    };
    ...
    // No-copy reserved-pool support: outstanding pool-writes per spill; when a spill's last write
    ...
    void diskWriterLoop();
    void enqueueDiskWrite(std::string filename, void const* src, std::size_t bytes, bool paid);
    void enqueueDiskWriteNoCopy(
