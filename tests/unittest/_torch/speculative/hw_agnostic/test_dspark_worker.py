...
def test_disagg_position_bootstrap_uses_actual_positions_and_target_width():
    ...
    worker._lazy_init(draft_model, metadata)
    # Exercise the eager draft core directly; the worker-owned draft graph needs
    # the full spec-metadata capture buffer this bare fixture does not provide.
...
def test_draft_gen_block_graphed_matches_eager():
    ...
    # Locally prefilled slots: the gen path must keep their advanced positions
    # rather than bootstrapping from the target's position_ids.
    worker._position_initialized[:3] = True
    worker._valid_len[:3] = torch.tensor([4, 9, 2], device="cuda")
    position_ids = torch.arange(total_target_tokens, device="cuda")

    win0, ctx0 = worker._kv_windows.clone(), worker._ctx_len.clone()
    vl0, pi0 = worker._valid_len.clone(), worker._position_initialized.clone()

    def run_block():
        return worker._draft_gen_block_batched(
            dm, meta, attn_metadata, accepted, num_accepted,
            num_contexts, batch_size, total_target_tokens, position_ids,
        )

    def reset_state():
        worker._kv_windows.copy_(win0)
        worker._ctx_len.copy_(ctx0)
        worker._valid_len.copy_(vl0)
        worker._position_initialized.copy_(pi0)

    # Eager reference (graph path disabled).
    worker._draft_graph_disabled = True
    ref = run_block().clone()
    win_ref, ctx_ref = worker._kv_windows.clone(), worker._ctx_len.clone()
    vl_ref = worker._valid_len.clone()
    ...
    for phase in ("warm", "capture", "replay"):
        ...
        assert torch.equal(worker._valid_len[:3], vl_ref[:3]), phase
