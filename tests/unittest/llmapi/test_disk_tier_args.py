...
"""Disk-tier plumbing unit tests: pydantic->pybind passthrough, retention pickle,
and the serve-layer TTL helper."""
import datetime
import pickle
...
def test_llm_args_kv_cache_config_disk_fields():
    ...
    py_cfg = PydanticKvCacheConfig(disk_cache_size=1 << 30,
...
def test_serve_disk_retention_helper():
    ...
    assert _disk_retention_config(Req()) is None  # ttl=0 behaves unpaid
