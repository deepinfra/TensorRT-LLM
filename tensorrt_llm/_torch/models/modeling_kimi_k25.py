...
class KimiK25ForConditionalGeneration(PreTrainedModel):
    ...
    # Spec-decode (MTP / Eagle3) delegations not already provided by the
    # upstream property block above: the drafter reads ``spec_worker`` /
    # ``spec_config`` off the top-level model, so route them to the LLM.
    @property
