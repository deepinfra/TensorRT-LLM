...
def dspark_attention_forward_batched(
    ...
    * the attended-position list has the fixed width ``window_size + block_size``
    ...
    if (
        ...
        if valid_len is None and g > 0 and _use_fused_draft_attn(x.device):
            # DeepInfra fused trtllm-gen path behind the per-geometry numeric
            # self-check (see the fused-draft-attention note above). It derives
            # window validity from start_pos alone, so it is only eligible when
            # no explicit valid_len mask is in play (same gate as the DSL path).
