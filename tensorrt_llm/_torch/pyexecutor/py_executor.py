...
class PyExecutor:
    ...
    def _park_requests_awaiting_onboard(self, scheduled_batch):
        ...
        if tp_sync:
            any_pending = self.dist.tp_allreduce(
                0 if all(local_ready) else 1, op=ReduceOp.MAX)
            if not any_pending:
                for req in ctx:
            ...
            if self.dist.tp_size > 1 and not getattr(self, "_warned_park_local_only", False):
                logger.warning(
        ...
        if parked:
            ...
            self._onboard_parked_total = getattr(self, "_onboard_parked_total", 0) + parked
