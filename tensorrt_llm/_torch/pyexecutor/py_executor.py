...
class PyExecutor:
    ...
    def __init__(
        ...
        # [kv-pool-init] Pre-fill primary KV cache pools after warmup.
        ...
        # as the pool warms up but never reaches zero.
        #
        # Default mode: fill with 0.0 (mitigation). Set TLLM_KV_POOL_INIT=nan
        # to fill with NaN instead — this is a positive-control diagnostic that
        # should produce IMMEDIATE, RELIABLE NaN crashes on the first request,
        ...
        if kv_pool_fill_mode == "nan":
            ...
            tag_word = "NaN-FILLING"
        elif kv_pool_fill_mode == "skip":
            fill_value = None  # leave uninit (current behavior pre-patch)
            tag_word = "SKIPPED"
        else:
            fill_value = 0.0
            tag_word = "zeroing"
        ...
        if fill_value is not None:
            for kvm in (self.kv_cache_manager,
                ...
                try:
                    ...
                    logger.info(
                        f"[kv-pool-init] {tag_word} {num_pools} primary pool(s) "
                        f"for {tag} kv cache manager")
                    try:
                        pool = kvm.impl.get_unique_primary_pool()
                        pool.fill_(fill_value)
                    except Exception:
                        num_layers = getattr(kvm, "num_layers", None) or 256
                        for layer_idx in range(num_layers):
                            try:
                                slice_t = kvm.impl.get_primary_pool_data(layer_idx)
                                slice_t.fill_(fill_value)
                            except Exception:
                                break
                except Exception as e:
                    logger.warning(
                        f"[kv-pool-init] could not fill pool for {tag} "
                        f"manager: {e}")
            torch.cuda.synchronize()
        logger.info(f"[kv-pool-init] done (mode={kv_pool_fill_mode})")
