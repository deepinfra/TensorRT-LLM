    ...
    def __init__(
        ...
        **kwargs,
        ...
        if is_estimating_kv_cache:
            ...
            # The dry run only measures GPU memory; skip the pinned host pool here,
            # its size can be miscomputed for hybrid models (12x over-request seen).
        ...
        if disk_cache_size:
            ...
            logger.info(
                f"[disk-tier] {blocks_in_disk_pool} blocks "
                f"({disk_cache_size / (1 << 30):.1f} GiB)")
        kwargs['blocks_in_disk_pool'] = blocks_in_disk_pool
        disk_cache_path = getattr(kv_cache_config, "disk_cache_path", None) or ""
        if disk_cache_path and blocks_in_disk_pool:
            ...
            disk_cache_path = os.path.join(disk_cache_path, f"rank_{self.mapping.rank}")
        ...
        kwargs['disk_cache_protect_unexpired'] = bool(getattr(kv_cache_config, "disk_cache_protect_unexpired", False))
            ...
            if batch_request_infos:
                try:
                    ...
                    if not (nonfatal and is_kv_cache_exhaustion_error(e)):
                        raise
                    # Onboarding overflowed the pool. Onboarding only touches
                    # first-chunk context requests (no in-flight forward
                    # references them yet), so reject the whole new-admission
                    # batch and let the running generation requests proceed.
